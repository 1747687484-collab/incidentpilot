...
- 展示模型工程能力：OpenAI-compatible Provider、prompt 模板、证据引用校验和失败回退。
...
- Agent RCA 支持可选模型生成，模型输出必须引用已存在证据。
- 模型调用失败、超时、JSON 不合格或证据引用不足时必须自动回退。
...
- 模型 API Key 不能进入 prompt、数据库、事件流或前端响应。
- 文档必须说明架构、分工、测试和验收标准。
...
- 启用模型 Provider 后，RCA 报告仍必须引用至少 2 条系统证据。
- 模型 Provider 不可用时，事故诊断仍能完成并产生确定性 RCA。
- 缓存击穿事故至少生成 4 条证据。
